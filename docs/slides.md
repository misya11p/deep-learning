...
機械学習モデルは、入力に対して適切な出力が行うように学習することが可能なモデルである。
上手く学習させると、以下のようなタスクをこなせるようになる。
...
先程の例で考えると、例えば画像は各画素の色を表す数値の集合で表せる。また画像分類の結果は、分類する種類の数だけ整数を割り当てて表せる。犬と猫の分類なら、どちらかに0、どちらかに1を割り当てるということ。
...
しかし、式の中の具体的な数値（$a$と$b$）は決まっていない。「入力に何かをかけて何かを足す」といった処理の内容は決まっているが、具体的に何をかけて何を足すかは決まっていないということ。この「何か（$a$, $b$）」はパラメータと呼ぶ。

このパラメータに最も適切な値を定めることが機械学習の目的である。

---

## ニューラルネットワーク

改めて、ニューラルネットワークは人間の脳の神経回路を模した機械学習モデルである。
